	...
	try:
		...
		if os.path.isfile('/proc/stb/fp/temp_sensor_avs'):
			...
			try:
				...
				pass
		if temperature:
			return (_("%s %s MHz (%s) %s") + ("\xb0C")) % (processor, cpu_speed, ngettext("%d core", "%d cores", cpu_count) % cpu_count, temperature)
		return _("%s %s MHz (%s)") % (processor, cpu_speed, ngettext("%d core", "%d cores", cpu_count) % cpu_count)
	except:
		return _("undefined")
