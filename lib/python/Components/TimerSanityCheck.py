...
# now process existing timers
		idx = 0
		for timer in self.timerlist:
			if timer != self.newtimer and not timer.disabled and (timer.conflict_detection or self.visible_conflict) and timer.service_ref and '%3a//' not in timer.service_ref.ref.toString():
				if timer.repeated:
