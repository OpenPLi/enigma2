...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport, TerrestrialTransponderSearchSupport):
	...
	def newConfig(self):
		...
		if cur == self.typeOfScanEntry or \
			...
			if self.t2mi_plp_id_boolEntry[1].value:
				...
				self.scan_sat.t2mi_pid.value = eDVBFrontendParametersSatellite.T2MI_Default_Pid if self.t2mi_plp_id_memory < 0 else self.scan_sat.t2mi_pid.value
			else:
	...
	def createConfig(self, frontendData):
		...
		self.scan_sat.t2mi_pid = ConfigInteger(default = defaultSat["t2mi_pid"], limits = (0, 8191))
	...
	def compareTransponders(self, tp, compare):
		...
		return abs(tp[1] - compare[1]) <= frequencyTolerance and abs(tp[2] - compare[2]) <= symbolRateTolerance and tp[3] == compare[3] and (not tp[4] or tp[4] == compare[4]) and (tp[5] == eDVBFrontendParametersSatellite.System_DVB_S or tp[10] == eDVBFrontendParametersSatellite.No_Stream_Id_Filter or tp[10] == compare[10] or tp[13] == compare[13])
