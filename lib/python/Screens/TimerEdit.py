	...
	def openExtendedSetup(self):
		...
		if cur:
			...
			if not cur.conflict_detection:
				timersanitycheck = TimerSanityCheck(self.session.nav.RecordTimer.timer_list, cur, True)
				if not timersanitycheck.check():
			...
			for timer in self.session.nav.RecordTimer.timer_list:
				if not timer.conflict_detection and (conflict_detection is None or conflict_detection != timer):
					timersanitycheck = TimerSanityCheck(self.session.nav.RecordTimer.timer_list, timer, True)
					if not timersanitycheck.check():
						simulTimerList = timersanitycheck.getSimulTimerList()
