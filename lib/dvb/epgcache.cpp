...
//			eDebug("[eEPGCache] event_id is %d sid is %04x", event_id, service.sid);
			...
			timeMap::iterator tm_it =
				servicemap.byTime.find(start_time);
			...
			else if (ev_erase_count == 0 && tm_erase_count > 0)
			{
				// exempt memory
				delete ev_it->second;
				tm_it = prevTimeIt = servicemap.byTime.insert( prevTimeIt, std::pair<const time_t, eventData*>( start_time, evt ) );
				ev_it->second = evt;
			}
			else if (ev_erase_count > 0 && tm_erase_count == 0)
			{
			...
			if ( consistencyCheck )
			{
				...
				else if ( tm_it->first != start_time )
					eFatal("[eEPGCache] timemap key(%d) non equal start_time(%d)",
						tm_it->first, start_time);
