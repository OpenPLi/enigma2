...
int eDVBSatelliteEquipmentControl::canTune(const eDVBFrontendParametersSatellite &sat, iDVBFrontend *fe, int slot_id, int *highest_score_lnb)
{
	...
	while (linked_prev_ptr != -1)
	{
		...
		eSecDebugNoSimulate("[eDVBSatelliteEquipmentControl] LINKED_PREV_PTR: %ld linked_in_use: %s", linked_prev_ptr, linked_in_use);
	}
	...
	while (!linked_in_use && linked_next_ptr != -1)
	{
		...
		eSecDebugNoSimulate("[eDVBSatelliteEquipmentControl] LINKED_NEXT_PTR: %ld linked_in_use: %s", linked_next_ptr, linked_in_use);
	}
