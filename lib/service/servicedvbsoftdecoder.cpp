...
int eDVBSoftDecoder::setupRecorder()
{
	...
	if (!m_record)
	{
		...
		int sync_mode_cfg = eConfigManager::getConfigIntValue("config.softcsa.syncMode", 0);
	...
	int wait_timeout = eConfigManager::getConfigIntValue("config.softcsa.waitForDataTimeout", 0);
